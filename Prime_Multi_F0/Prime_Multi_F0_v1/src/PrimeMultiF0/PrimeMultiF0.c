 ...
 * @param n, IS THE ARRAY OF WINDOWS TO PROCESS
 * @param Xz, THE ZERO PADDED SIGNAL
 * @param w,
...
void getWsScoreMat_PrimeMultiF0(int i, vector x, vector ws, vector pc, vector d, matrix S, double fs, vector p0, vector t){
	...
	// Zero pad signal is now a vector
...
matrix primeMulti_F0(char wav[], double min, double max, double dt){
	...
	for(i = 0; i < ws.x; ++i){
		getWsScoreMat_PrimeMultiF0(i, x, ws, pc, d, S, fs, p0, t);
	}
