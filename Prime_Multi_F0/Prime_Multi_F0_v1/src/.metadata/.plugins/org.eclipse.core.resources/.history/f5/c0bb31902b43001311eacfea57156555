...
matrix primeMulti_F0(char wav[], double min, double max, double dt){
	printf("INICIO PRIME MULTI F0\n");
	vector t, ws, logWs, p0, log2pc, pc, d, x;
	matrix S;
	double a, dlog2p, dlog2p_max, nyquist, nyquist2, fs;
	int i, tamX, rango, maxvent, minvent, frames;
	//x contains the sound signal, reads the signal from file
	...
	//Testing
	//getWsScoreMat(0, ws, pc, d, S, fs, p0, t);
	outBinaryV(pc.v, pc.x, "pc.xlx");
	outBinaryV(d.v, d.x, "d.xlx");
	outBinaryV(p0.v, p0.x, "p0.xlx");

	return S;

}

void getWsScoreMat_PrimeMultiF0(int i, vector ws, vector pc, vector d, matrix S, double fs, vector p0, vector t){
